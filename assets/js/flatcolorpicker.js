...
<div class="btn-group"><button class="btn btn-default active">Name</button></div>\
</div>\
</div></div>\
  ...
  FlatColorPicker.prototype.insertColor = function () {
	...
	hex     = '';
  	tip.on('click','.'+c+'-item',function(e) {
		...
		$('.output,.add-text,.input-text > span').css({'font-family':$('.font_hidden').val(),'color':hex.toUpperCase(),'font-size':parseInt($('.size_hidden').val())+'px'});	
		$('.color_hidden').val(hex.toUpperCase());
